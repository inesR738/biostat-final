...
Regardons si notre jeu de données contient des données manquantes :
...
Nous allons d'abord observer la répartition de l'âge des hommes et des femmes au sein de l'échantillon.

```{r, echo=FALSE}
ggplot(Intoxications, aes(group = Sexe, x = Sexe, y = Age, fill = Sexe)) +
  geom_boxplot() +
  ggtitle("Répartition de l'âges selon le sexe") +
...
Nous allons chercher le meilleur modèle possible pour expliquer cette variable $Malade$, au vu du critère d'Akaike définit comme : $$AIC = -2ln(V) + 2k$$ Avec $k$ le nombre de paramètres et $V$ la vraisemblance.
